...
### Enhanced Modules
- **Dashboard** - Dynamic salary countdown based on user's payday config, today's working status display, time-based greetings; **NEW: Live clock** with real-time updating seconds; **NEW: Privacy toggle** to hide/show financial amounts (income, expenses, balance); **SYNC: Financial data now includes recurring templates** - income/expense/balance reflect both transactions and recurring items
